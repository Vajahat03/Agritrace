...
export function CropCard({ crop, onAddFertilizer, onLogHarvest }: CropCardProps) {
  ...
  return (
    <div className="rounded-2xl glass-card-interactive overflow-hidden flex flex-col group border border-slate-200">
      {/* Image Header */}
      <div className="relative h-44 w-full overflow-hidden bg-emerald-950">
        ...
        {/* Crop Area */}
        <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-full text-xs font-medium text-white border border-white/30">
      ...
      <div className="p-4 flex-1 flex flex-col justify-between space-y-4">
        <div className="space-y-2 text-xs text-slate-700">
          ...
          <div className="flex items-center justify-between border-t border-slate-200 pt-2">
            ...
            <span className="font-medium text-slate-800">{crop.planting_date}</span>
          </div>
          ...
          {crop.expected_harvest_date && (
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-amber-800">{crop.expected_harvest_date}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2 pt-2 border-t border-slate-200">
          <div className="grid grid-cols-2 gap-2">
            {onAddFertilizer && (
              <button
                ...
                className="flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 text-xs font-medium transition-colors"
              >
            ...
            {onLogHarvest && (
              <button
                ...
                className="flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 text-xs font-medium transition-colors"
              >
          ...
          <Link
            ...
            className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-semibold text-xs transition-all shadow-sm"
          >
